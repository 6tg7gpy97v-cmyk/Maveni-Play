...
function send(res,status,body,type='application/json'){res.writeHead(status,{'Content-Type':type,'Cache-Control':'no-store','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'Content-Type','Access-Control-Allow-Methods':'GET,POST,OPTIONS'});res.end(typeof body==='string'?body:JSON.stringify(body));}
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='GET'&&req.url==='/health') return send(res,200,{ok:true,aiConfigured:Boolean(process.env.OPENAI_API_KEY),model:'gpt-5'});
    if(req.method==='POST'&&req.url==='/api/recognize'){
      ...
      const api=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Authorization':'Bearer '+process.env.OPENAI_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5',input:[{role:'user',content:[{type:'input_text',text:prompt},{type:'input_image',image_url:image,detail:'auto'}]}]})});
